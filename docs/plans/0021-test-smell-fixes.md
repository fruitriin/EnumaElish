...
| # | 対応内容 |
|---|---|
| 1,2 | `findAuditLine` ヘルパー抽出で `for range` + `if` を解消 |
| 3 | `countTokenType` ヘルパー抽出で `for range` + `if` を解消 |
| 4 | `TestParseBasicRules` をルール別サブテスト3つに分割 |
| 5 | `return` → `t.Skip` + `smell-allow: ignored-test` |
| 6,7 | アサーション追加 + `smell-allow: conditional-test-logic` |
| 8 | `evaluateAllCommands` + `assertRulesetCharacteristics` にヘルパー抽出 |

**結果**: 8件 → 0件（+ 2件 smell-allow）
