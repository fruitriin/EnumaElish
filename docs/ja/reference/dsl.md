...
- シェルのクォートは、シェルがコマンド実行前に除去するのと同様、マッチ前に除去される: `curl -X "POST"` はパターン `-X POST` にマッチする
- 結合した引数文字列が **4096 バイト**を超える場合、args: ルールは適用されず結果は `ask` にエスカレーションされる（親アクションが `deny` 等より厳格な場合はそちらを維持）。親アクションへのフォールバックにすると、引数のパディングでエスカレーション系 args: ルールをバイパスできてしまうため
