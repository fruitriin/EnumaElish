...
## タスク: Plan 0003 — ccchain ルール評価エンジンと Hook 統合

### Phase 1: ルール評価エンジン
- [x] `internal/eval/evaluate.go` — Result 型、Evaluate 関数、マッチングアルゴリズム
- [x] テンプレート展開（next/extends）を考慮したマッチング
- [x] `internal/eval/evaluate_test.go` — テーブル駆動テスト（12テスト）

### Phase 2: Hook 統合
- [x] `cmd/ccchain/hook.go` — `ccchain hook pre` / `ccchain hook post` サブコマンド
- [x] stdin からツール情報 JSON を読み、exit code で結果を返す
- [x] `cmd/ccchain/main.go` に hook/eval サブコマンドのルーティング追加

### Phase 3: eval サブコマンド
- [x] `cmd/ccchain/eval_cmd.go` — `ccchain eval "cmd"` で評価結果を JSON 出力

### Phase 4: ベンチマーク
- [x] `internal/eval/bench_test.go` — Evaluate + EndToEnd ベンチマーク

### Phase 5: 品質ゲート
- [x] `go test ./...` 全パス
- [x] `go vet ./...` パス
- [x] `go build ./cmd/ccchain` でバイナリ生成確認
- [x] ベンチマーク: Evaluate 3.0μs, EndToEnd 5.4μs（目標5msの1000倍速）
