...
## 意図的完了の明記（2026-07-06・Plan 0036 埋没監査 #10 の回収）

- 「拡張アイデア: 深さの投機・MVP バリエーション実験」は**意図的な先送り**として明記して閉じる（オーナー指示 B3）
- 「深さの投機」= 深化ブランチは Plan 0035 フェーズB で実装済み。「MVP バリエーション実験」は判断→採否のトリガーが未定（実運用で需要が観測されたら再起票）

## 未決事項の決定記録（2026-07-06・Plan 0036 埋没監査 #9 の回収 = 参照訂正）

- 起票時に残っていた「.claude 複製の実現手段」の未決事項は、**実装で明示コピー（cp -r + find による除外削除 + git checkout での追跡ファイル復元）に確定済み**（Issue #18 対応・v0.5.0 リリース済み）。本節は「決定記録の反映漏れ」の解消のため事後追記
