...
## タスク: Plan 0007 — テスト基盤整備（リネーム実施済み、テスト作成へ）

- [x] 1. addToolsSrc → addfTools リネーム（実施済み）
- [x] 2. テストディレクトリ構成作成
- [x] 3. スキルテスト（自然言語シナリオ 7件）
- [x] 4. フックテスト（シェルスクリプト 2件、全13アサーション PASS）
- [x] 5. ツールテスト（バイナリ疎通 4件、全8アサーション PASS）
- [x] 6. テストランナー（run-all.sh、全テスト PASS）
- [x] 7. 品質検証（テスト自体がテスト）
- [x] 8. 完了処理
