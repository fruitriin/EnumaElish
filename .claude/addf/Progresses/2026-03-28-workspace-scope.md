...
6. `addf-code-review-agent` でコードレビューを実施する
7. `addf-doc-review-agent` でドキュメントレビューを実施する
8. `addf-contribution-agent` で ADD フレームワークへのコントリビューション候補を検出する
9. レビュー指摘への対応:
...
13. `.claude/addf/Progresses/YYYY-MM-DD-プラン名.md` にリネームして移動し、`.claude/addf/templates/ProgressTemplate.addf.md` から新規の Progress.md を作成する
14. Progress 推進エンジン自体に関するフィードバック・ノウハウがあれば、テンプレート（`.claude/addf/templates/ProgressTemplate.addf.md`）の改善案を `.claude/addf/Feedback.md` に記録する
...
Plan 0011 — ワークスペーススコープ v1

### 実装
- [ ] DSL: Settings に WorkspacePaths 追加、scope: ディレクティブパース
- [ ] eval/scope.go: パス分類ロジック（Critical/High 対策込み）
- [ ] eval/tool.go に scope 判定を統合
- [ ] eval/evaluate.go の Bash コマンド評価にも scope 判定追加

### テスト
- [ ] scope 判定テスト（inside/outside/traversal/tilde bypass）
