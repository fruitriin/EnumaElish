...
## タスク: Plan 0006 — コード品質リファクタリング

### Warning
- [x] W-1: SegmentType 型定数化
- [x] W-2: EvaluateTopology nil Settings 対応
- [x] W-3: template.go の死コード除去
- [ ] W-4: Audit exec truncation 修正（Plan 0007 へ先送り）
- [ ] W-5: PostRules 未評価の明示（Plan 0007 へ先送り）

### Suggestion
- [ ] S-1: テンプレート収集ロジック移動（Plan 0007 へ先送り — 影響範囲大、独立タスク）
- [x] S-2: LookupTemplate O(1) 化（Config.TemplateIndex 追加）
- [x] S-3: main.go blank identifier 削除
- [ ] S-4: parseArgsBlock エラー（Plan 0007 へ先送り）
- [x] VULN-09: next: 循環の静的検出（checkCircularNext 追加）

### 品質ゲート
- [x] `go test ./...` + `go vet ./...` 通過
- [x] ベンチマーク回帰なし（EndToEnd: 5.4μs→4.3μs、LookupTemplate O(1) 化で改善）
