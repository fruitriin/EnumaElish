...
11. `.claude/addf/Progresses/YYYY-MM-DD-プラン名.md` にリネームして移動し、`.claude/addf/templates/ProgressTemplate.md` から新規の Progress.md を作成する
12. Progress 推進エンジン自体に関するフィードバック・ノウハウがあれば、テンプレート（`.claude/addf/templates/ProgressTemplate.md`）の改善案を `.claude/addf/Feedback.md` に記録する

13. コミットする

---

## タスク: Plan 0004 — GUI テストのクロスプラットフォーム抽象化

- [x] 1. `add-Behavier.toml` → `add-Behavior.toml` リネーム
- [x] 2. Swift ソース内の `add-Behavier.toml` 参照を `add-Behavior.toml` に更新
- [x] 3. `add-Behavior.toml` に `machine` 設定を追加
- [x] 4. `add-gui-test.md` + optional 版にプラットフォーム判定ロジックを追加
- [x] 5. README.md / README.en.md の `add-Behavier.toml` 参照を更新
- [x] 6. GUI テスト セットアップドキュメント（.claude/addf/guides/gui-test-setup.md）を作成
- [x] 7. Swift バイナリを再ビルド（全4ツール成功）
- [x] 8. TODO.md を「実施中」に更新
- [x] 9. 品質検証（コードレビュー: Suggestion 3件対応）
- [x] 10. 完了処理
