...
## タスク: Plan 0001 — ccchain DSL 設計とパーサー実装

### Phase 1: Go プロジェクト初期化
- [x] `go mod init` + ディレクトリ構造作成
- [x] `mvdan.cc/sh` を依存に追加
- [x] `Makefile` 作成（build, test, bench ターゲット）
- [x] `cmd/ccchain/main.go` スケルトン（サブコマンドルーティング）

### Phase 2: DSL AST 定義
- [x] `internal/dsl/ast.go` — AST ノード型定義（Config, Rule, Template, Settings 等）

### Phase 3: レキサー
- [x] `internal/dsl/lexer.go` — インデント解析、トークン化
- [x] `internal/dsl/lexer_test.go` — トークン列テスト

### Phase 4: パーサー
- [x] `internal/dsl/parser.go` — 再帰降下パーサー（ルール、テンプレート、設定セクション）
- [x] テストフィクスチャ作成（`testdata/dsl/*.conf`）
- [x] `internal/dsl/parser_test.go` — フィクスチャベースのパーサーテスト

### Phase 5: テンプレート解決
- [x] `internal/dsl/template.go` — extends/next 解決、循環参照検出
- [x] `internal/dsl/template_test.go` — 継承チェーン・循環参照テスト

### Phase 6: 設定ファイル読み込みと CLI
- [x] 設定ファイル探索ロジック（.ccchain.conf → .local → CLAUDE_CONFIG_DIR → ~/.claude/）
- [x] `ccchain check` サブコマンド
- [x] CLI 共通フラグ（--verbose, --quiet, --config）

### Phase 7: ベンチマーク
- [x] `internal/dsl/bench_test.go` — Lexer/Parser/TemplateResolve ベンチマーク

### Phase 8: 品質ゲート
- [x] `go test ./...` 全パス
- [x] `go vet ./...` パス
- [x] `go build ./cmd/ccchain` でバイナリ生成確認
- [x] ベンチマーク実行・結果確認（Lexer: 3.6μs, Parser: 4.7μs, Resolve: 5.0μs — 目標1ms大幅クリア）
