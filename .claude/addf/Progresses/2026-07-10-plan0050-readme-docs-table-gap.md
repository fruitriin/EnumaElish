...
### 現在のタスク: Plan 0050（README.md「ドキュメント」テーブルの掲載漏れ修正）

#### サブタスクチェックリスト

- [x] Feedback.md・TODO.addf.md・Progress.md を確認（前サイクル Plan 0039 フェーズ2 は完了・引き継ぎ事項なし）
- [x] README.md の「ドキュメント」テーブルを確認し、実際の `.claude/addf/guides/` 一覧と突合
- [x] README.md に skills.md・model-allocation.md の行を追加
- [x] README.en.md に対応する行を追加
- [x] Plan 0050・TODO.addf.md を更新
- [x] 品質ゲート（run-all.sh・lint）→ doc-review 起動
- [x] レビュー指摘への対応（Critical/Warning なし。Info: README⇔guides突合のlint不在は改善候補として認識のみ、対応不要と判断）
- [x] ノウハウ記録・Feedback 記録（該当なし。新規知見なしと判断）
- [x] Progress アーカイブ・最終コミット

#### 日記

##### 2026-07-10 — Plan 0050 は前サイクルの doc-review 由来の軽微な修正
**やったこと**: cron 経由の `/loop` 4回目。前サイクル（Plan 0039 フェーズ2）の doc-review が
発見し切り出した Plan 0050 に着手。README.md の「ドキュメント」テーブルに8ガイドしか
掲載されておらず、実際の `.claude/addf/guides/` には10ファイルあることを確認。
**今の見立て**: 軽微な修正のため、コードレビュー・doc-review はスキップ判断もありうるが、
README変更は doc-review 起動条件（`*.md` 変更）に該当するため通常どおり起動する。
**次の自分へ**: README.en.md にも対応する英語行を追加してから品質ゲートへ。
**気になっていること**: なし。
