...
出典: `.claude/addf/plans-add/0033-downstream-reported-fixes.md`。共通根因「ファイルの存在で upstream/downstream を判定」を明示シグナル（addf-lock.json / CLAUDE.repo.md 宣言）に統一する。項目4（PlanTemplate 新設）は独立・低優先のため本サイクルから切り離し、次サイクル以降に実施。
