...
## タスク: Plan 0004 — ccchain 監査出力・デフォルトルールセット・配布

### Phase 1: 監査出力エンジン
- [x] `internal/audit/audit.go` — フラット展開ロジック（テンプレート展開、打ち切り制御）
- [x] `internal/audit/audit_test.go` — テスト（4テスト）
- [x] `cmd/ccchain/audit.go` — `ccchain audit` サブコマンド

### Phase 2: init サブコマンドとデフォルトルールセット
- [x] `cmd/ccchain/init_cmd.go` — `ccchain init` サブコマンド（.ccchain.conf 生成）
- [x] デフォルトルールセットの組み込み

### Phase 3: main.go ルーティング追加
- [x] audit / init サブコマンドの追加

### Phase 4: テストフィクスチャとベンチマーク
- [x] `internal/audit/bench_test.go` — ベンチマーク（Audit: 2.8μs, Format: 17μs）

### Phase 5: 品質ゲート
- [x] `go test ./...` 全パス
- [x] `go vet ./...` パス
- [x] `go build ./cmd/ccchain` でバイナリ生成確認
- [x] デフォルトルールセットでのシナリオ検証（curl|bash→deny, ls|head→allow, find-exec rm→deny）
