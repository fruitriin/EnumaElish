...
### 現在のタスク: Plan 0038 — 投機適性の概念化と大改造の窓

出典: `.claude/addf/plans-add/0038-speculation-fitness.md`（オーナー起案 2026-07-05）。0035 から「Pending 在庫・深化カスケードの機械化検討」が申し送りされている。0037（one-shot 第1号）の前提。

#### サブタスクチェックリスト

- [ ] 投機適性3区分（向き/不向き/禁止）を addf-speculate 選定手順と guides に追加
- [ ] 不適合→Plan 化フォールバック（silent 廃棄禁止・one-shot マーカー付き起案・Dashboard 記録）
- [ ] one-shot マーカー定義を guides に単一ソース化（0037 実施様式の一般化）＋ PlanTemplate 反映
- [ ] 窓検出（発動ガード後の判定: 在庫ゼロ ∧ one-shot Plan あり → 選択提示。応答待ち中は新規投機を控え3サイクルで解除）
- [ ] 0035 申し送り（Pending 在庫上限・深化カスケードの機械化）の採否判断
- [ ] Stage 1 → 3体レビュー → 修正 → マージ → 完了化 → アーカイブ → コミット

#### 日記

##### 2026-07-05 — /loop サイクル5開始（0038。誤キャンセル→speculate 誤起動を経て復帰）
**やったこと**: オーナーの誤操作で /addf-speculate が起動されたが「気にしないで」指示で 0038 に復帰。副産物: guard/reconcile 実測で**投機在庫ゼロ＝窓が開いている**ことを確認（0038 の窓検出設計の生きた実例）。detached worktree（mystifying-jepsen-088bd4）残骸を発見・報告済み（削除はオーナー判断）。
**今の見立て**: 0038 はドキュメント中心（スキル・guides・テンプレート）。新スクリプト原則なし（在庫ゼロ判定は reconcile 出力を使う設計）。
**次の自分へ**: 委譲→3体→修正→マージ→アーカイブの型（5サイクル目）。完了後は 0037 のみ残るが、0037 は one-shot・オーナー同席必須なので**自走サイクルでは着手しない**（窓検出の提案どおりオーナーに提示して待つ）。ループの次サイクルはアイドル→投機になる想定。
**気になっていること**: コンテキスト 435k+。compaction 必至 — この日記が正。
