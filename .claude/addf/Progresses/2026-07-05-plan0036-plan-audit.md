...
### 現在のタスク: Plan 0036 — /addf-plan-audit スキル（埋没タスク掘り起こし）

出典: `.claude/addf/plans-add/0036-plan-audit-skill.md`（オーナー起案 2026-07-05）。0035 から「関連 Plan 風化検出（意味的）」が申し送りされている。

#### サブタスクチェックリスト

- [ ] /addf-plan-audit スキル新設（走査3層: 構造=lint-plan-status 呼び出し・意味的パターン・TODO 突合。処置3択の提案まで、完了状態変更はオーナー判断 human-judgment）
- [ ] addf-migrate のバージョン差分手順にワンショット実行ステップ（参照のみ・処理を埋め込まない）
- [ ] addf-init コピーリスト・lint 対象（frontmatter 等）への追加要否確認
- [ ] ドッグフーディング: 本体 Plan 0001〜0038 へ適用し初回掘り起こし実測（受け入れテスト。Plan 0026 の手動掘り起こしと比較）
- [ ] Stage 1 → 3体レビュー → 修正 → マージ → Plan 0036 完了化 → アーカイブ → コミット

#### 日記

##### 2026-07-05 — /loop サイクル4開始（0036 委譲）
**やったこと**: タスク選択（Plan 0036。実装順合意 0035→0036→0038→0037 の2番目）。worktree 委譲＋3体レビューの型を継続（8サイクル連続で初版 Critical 実測の実績）。
**今の見立て**: スキル .md 新設中心＋ドッグフーディング実行。lint-plan-status（サイクル3で新設）をスキルの構造検査層として呼び出す設計になるはず。
**次の自分へ**: 委譲完了通知 → 3体レビュー → 集約・修正 → squash マージ → 完了処理。ドッグフーディングの検出結果は Dashboard/報告に載せ、**完了状態の変更はオーナー判断なので実施しない**こと。
**気になっていること**: コンテキストが極めて深い（370k+）。compaction 後はこの日記とチェックリストが正。cron 608e6a9a 継続中。
