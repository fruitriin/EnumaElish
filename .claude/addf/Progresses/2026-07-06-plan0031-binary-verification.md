...
### 現在のタスク: Plan 0031 — コミット済みバイナリの検証可能性（チェックサム照合）

#### サブタスクチェックリスト

- [ ] build.sh に checksums.sha256 生成追加・checksums 初回生成（macOS なので再ビルド検証も可）
- [ ] 照合テスト新設（全 OS 実行・sha256sum/shasum フォールバック・片側コミット検出）
- [ ] 未決4件の判断（checksums 不在=ダウンストリーム SKIP/本体 FAIL 推奨・ビルド決定性は実験・バイナリ外し案は評価のみ）
- [ ] addf-init コピーリスト整合確認
- [ ] Stage 1 → 3体レビュー → 修正 → マージ → CI 確認 → 完了化 → アーカイブ

#### 日記

##### 2026-07-05 — /loop サイクル7開始（0031 バイナリ検証）
**やったこと**: タスク選択（0031。残り 0031/0032 の若番。CI=0030 が先行済みなので照合テストがそのまま CI に乗る）。
**次の自分へ**: 委譲→3体→修正→マージ→push 後 CI 緑確認→アーカイブの型（7サイクル目）。Swift 再ビルドの決定性実験は macOS の当環境で可能。完了後は 0032 のみ、0037 はオーナー同席待ち、0030 は branch protection 判断待ち。コンテキスト 552k — compaction 後はこの日記が正。cron 608e6a9a。
**気になっていること**: なし。
