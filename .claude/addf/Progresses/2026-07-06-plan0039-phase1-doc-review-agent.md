...
### 現在のタスク: Plan 0039 フェーズ1 — addf-doc-review-agent の逆輸入

#### サブタスクチェックリスト

- [ ] EnumaElish 版を汎用化して .claude/agents/ に取り込み（プロジェクト固有例は「書き方の例示」に降格・汎用観点3つを主軸）
- [ ] ProgressTemplate.addf.md / ProgressTemplate.md の品質ゲートに「ドキュメントに触れた変更のみドキュメントレビュー起動」を追加（同期ペア維持）
- [ ] addf-init コピーリスト整合（ペア5 通過確認）
- [ ] ドリフト注入テスト（実在した乖離パターンを再現）
- [ ] Stage 1 → 3体レビュー → 修正 → マージ → CI 緑 → 完了化（フェーズ1のみ）→ アーカイブ

#### 日記

##### 2026-07-06 — /loop サイクル9開始（0039 フェーズ1）
**やったこと**: タスク選択（0039 フェーズ1・独立先行可）。EnumaElish に addf-doc-review-agent.md 実物ありを確認。
**次の自分へ**: 委譲→3体→修正→マージ→CI→アーカイブの型（9サイクル目）。フェーズ2/3 は 0037 完了後（オーナー同席待ち）なので今回はフェーズ1のみで Plan は「一部完了」。ダウンストリーム配布物なので汎用化が要（EnumaElish 固有の printUsage 突合等は「例示」に降格）。0038 の窓検出（在庫ゼロ ∧ one-shot Plan）は今エージェントバックログに 0037 があるが、通常タスク=0039 フェーズ1 があるうちは窓提示のタイミングではないと判断（addf-speculate 発動時の話であって addf-dev のタスク選定順ではない）。
**気になっていること**: doc-review-agent はコードレビューと違い「意味的乖離」（実装済み機能の「未実装」注記残留等）を検出するのが目的。3体ペルソナ並列を使うかは委譲後の実装エージェントの判断でよい — ここでは通常タスクの単体レビューで進める。
