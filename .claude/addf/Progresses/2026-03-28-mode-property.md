...
6. `addf-doc-review-agent` でドキュメントレビューを実施する
7. `addf-contribution-agent` で ADD フレームワークへのコントリビューション候補を検出する
8. レビュー指摘への対応:
...
10. `.claude/addf/Feedback.md` にPlan, TODO, Progress推進エンジンの問題の記録・改善アクションを追記する。反映済みの項目は削除する
11. `.claude/addf/Feedback.md` にプロジェクト進行上の問題の記録・改善アクションを追記する。反映済みの項目は削除する
12. `.claude/addf/Progresses/YYYY-MM-DD-プラン名.md` にリネームして移動し、`.claude/addf/templates/ProgressTemplate.addf.md` から新規の Progress.md を作成する
...
## タスク: Plan 0009 — mode プロパティ + ドキュメント正確性

### Phase 1: ドキュメント修正
- [ ] actions.md (EN/JA): warn の例を `warn curl "..."` に修正
- [ ] dsl.md (EN/JA): mode: プロパティに deprecated 注記

### Phase 2: mode deprecated 化
